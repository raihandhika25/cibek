...
@section('konten')
    <div class="container-fluid">
        ...
        <table class="table table-dark table-hover m-lg-2">
            ...
            @foreach ($pembelian as $p)
                    ...
                    @if (Auth::user()->role != 'Guest')
                        <td>
                            <button class="btn btn-danger" data-bs-toggle="modal"
                                data-bs-target="#ModalDeletepembelian{{ $p->kode_pembelian }}">Delete</button>
                        </td>
                    @endif
                </tr>

                <!-- Ini tampil form update pembelian -->
                <div class="modal fade" id="ModalUpdatepembelian{{ str_replace('/', '', $p->kode_pembelian) }}"
                            ...
                            <div class="modal-body">
                                <form action="/pembelian/storeupdate" method="post" class="form-floating">
                                    ...
                                    <div class="form-floating p-1">
                                        <input type="text" name="kode_pembelian" id="kode_pembelian" readonly
                                            class="form-control" value="{{ $p->kode_pembelian }}"> <label
                                            for="floatingInputValue">Kode Pembelian</label>
                                    </div>
                                    <div class="form-floating p-1">
                                        <select name="status" class="form-control">
                                            <option value="verifikasi"
                                                @if ($p->status == 'verifikasi') selected="selected" @endif>Verifikasi
                                            </option>
                                            <option value="proses"
                                                @if ($p->status == 'proses') selected="selected" @endif>Proses
                                            </option>
                                            <option value="kirim"
                                                @if ($p->status == 'kirim') selected="selected" @endif>Kirim
                                            </option>
                                            <option value="selesai"
                                                @if ($p->status == 'selesai') selected="selected" @endif>Selesai
                                            </option>
                                        </select>
                ...
                <div class="modal fade" id="ModalDeletepembelian{{ $p->kode_pembelian }}" tabindex="-1"
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form action="/pembelian/delete/{{ $p->kode_pembelian }}" method="get"
                                    class="form-floating"> @csrf <div>
    ...
    <div class=" modal fade" id="ModalTambahpembelian" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
